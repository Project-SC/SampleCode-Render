...
{
	// calculate the distance between the camera and edge of the vertex
	float d = distance(p, eye.xyz);

	// minimum range
	const float d0 = 16.0f;

	// maximum range
	const float d1 = 2000.0f;

	// normalize the distance and chage it to a value of 0~1
    // if the tesselation value is far, it must be small, and if it is close, it must be large. so 1 is subtracted.
	float s = 1.0f - saturate((d - d0) / (d1 - d0));

	// tessellation step is linearly convert into step 1~6
	// maximum is 32, so it is power of 2
	return pow(2, (lerp(1, 6, s)));
}
	...
	uint PatchID : SV_PrimitiveID)
{
	...
	{
		//don't tessellatesss any part of the skirt that isn't directly touching the terrain.
		if (output.skirt == 0) 
		{
