...
GeometryData::GeometryData(ID3D11Device* device, ID3D11DeviceContext* deviceContext, KdTree* treeToUse)
	: tree(treeToUse)
{
	...
	LoadBitmapHeightMap("./Assets/heightmap.bmp");
	//LoadPngHeightMap("./Assets/heightmap.png");
	GenerateTerrainData();
	//GenerateCubeData(60,60,60);
...
bool GeometryData::LoadBitmapHeightMap(const char* filename)
{
	int error, imageSize, k;
	FILE* filePtr;
	...
	m_depth = bitmapInfoHeader.biWidth;

	m_heightMap = new HeightMapType[m_terrainWidth * m_terrainHeight];
	if (!m_heightMap)
		return false;

	imageSize = m_terrainWidth * m_terrainHeight * (bitmapInfoHeader.biBitCount / 8);
	...
	for (int j = 0; j < m_terrainHeight; j++)
	{
		...
		{
			unsigned char depth = bitmapImage[k];

			int index = (m_terrainHeight * j) + i;
			//int index = m_terrainWidth * (m_terrainHeight - 1 - j) + i;

			m_heightMap[index].x = (float)i;
			m_heightMap[index].y = (float)j;
			m_heightMap[index].z = (float)depth;

			k += bitmapInfoHeader.biBitCount / 8;
	...
	m_depth = (unsigned int)maxDepth - (unsigned int)minDepth + 1;
	if (m_depth < m_width)
		m_depth = m_width;
...
void GeometryData::LoadPngHeightMap(const char* filename)
{
	unsigned int h, w;
	unsigned char* data;

	lodepng_decode32_file(&data, &w, &h, filename);
	...
	m_heightMap = new HeightMapType[w * h];
	...
	m_depth = (unsigned int)maxDepth - (unsigned int)minDepth;
	if (m_depth > m_width)
		m_depth = m_width;
...
void GeometryData::GenerateTerrainData()
{
	m_data = new float[m_terrainWidth * m_terrainHeight * m_depth];
	//int memSize = m_terrainWidth * m_terrainHeight * m_depth;
	//m_data = (float*)malloc(sizeof(float) * (m_terrainWidth * m_terrainHeight * m_depth));
	//m_data = (float*)calloc(memSize, sizeof(float));
	...
	for (size_t z = 0u; z < m_depth; ++z)
	{
		for (size_t y = 0u; y < m_height; ++y)
		{
			...
			{
				int heightIndex = (m_height * z) + x;

				if (
					x >= 0 && x <= m_width
					&& y >= 0 && y <= m_heightMap[heightIndex].z
					&& z >= 0 && z <= m_depth)
				{
					m_data[index] = 1.0f;
...
void GeometryData::NormalizeHeightMap()
{
	for (int j = 0; j < m_terrainHeight; j++)
	{
		for (int i = 0; i < m_terrainWidth; i++)
		{
			m_heightMap[(m_terrainHeight * j) + i].z /= 15.0f;
		}
	}
}

void GeometryData::GenerateCubeData(unsigned width, unsigned hegight, unsigned depth)
{
	m_data = new float[width * hegight * depth];
...
void GeometryData::LoadTextures(ID3D11Device* device)
{
	...
	rock->Initialize(device, L"./Assets/rock.dds", L"./Assets/rockheightmap.dds");
...
bool GeometryData::InitializeShaders(ID3D11Device* device)
{
...

	{
		...
		bufferDesc.BindFlags = D3D11_BIND_STREAM_OUTPUT | D3D11_BIND_VERTEX_BUFFER; // Wichtig ist du musst beide Flags setzen, einmal f? Output und einmal f? vertex buffer.
