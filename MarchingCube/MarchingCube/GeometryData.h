...
{
public:
	GeometryData(ID3D11Device* device, ID3D11DeviceContext* deviceContext, KdTree* treeToUse);
	~GeometryData();
...
private:
	...
	void GenerateCubeData(unsigned width, unsigned hegight, unsigned depth);

	bool LoadBitmapHeightMap(const char* filename);
	void LoadPngHeightMap(const char* filename);
